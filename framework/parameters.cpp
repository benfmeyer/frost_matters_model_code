...
void initpft(Pft& pft,xtring& setname) {
	...
	/// BEN M. FROST
	/// set late-frost tolerance so that PFT is never susceptible to frost (can be overwritten in ins file)
	pft.frost_tolerance = -1000.0;
}
...
void plib_declarations(int id,xtring setname) {
	...
	case BLOCK_GLOBAL:
		...
		// Ben M. frost flag
            declareitem("iffrost",&iffrost,1,CB_NONE,
                        "Whether to simulate late-frost");
	...
	case BLOCK_PFT:
		...
		/// BEN M. FROST
		declareitem("frost_tolerance",&ppft->frost_tolerance,-20.0,0.0,1,CB_NONE,
              "Minimum temperature tolerance to spring late-frost");
            declareitem("frost_sd", &ppft->frost_sd, 0, 20, 1, CB_NONE, "Standard deviation of frost tolerance around the mean");
