...
void respiration(double gtemp_air, double gtemp_soil, lifeformtype lifeform,
	...
	double cmass_sap, double cmass_root_today, double assim, double& resp, bool was_frost) {
	...
	if (lifeform == TREE) {
        ...
        //cton_sap = cton_sap - (cton_sap * 0.31 * was_frost); // decrease cton ratio when there was frost
        //cton_root = cton_root - (cton_root * 0.31 * was_frost); // Baumgarten et al 2023
...
void npp(Patch& patch, Climate& climate, Vegetation& vegetation, const Day& day) {
	...
	while (vegetation.isobj) {
		...
		respiration(gtemp, patch.soil.gtemp, indiv.pft.lifeform,
			...
			indiv.cmass_sap, cmass_root, assim, resp, indiv.was_frost);
