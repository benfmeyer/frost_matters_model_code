...
CommonOutput::CommonOutput() {
    ...
    // BEN_FROST ==>
    declare_parameter("file_cmass_leaf", &file_cmass_leaf, 300, "cmass leaf output file");
    declare_parameter("file_cmass_heart", &file_cmass_heart, 300, "cmass heart output file");
    declare_parameter("file_cmass_root", &file_cmass_root, 300, "cmass root output file");
    declare_parameter("file_cmass_sap", &file_cmass_sap, 300, "cmass sap output file");
    declare_parameter("file_frost", &file_frost, 300, "frost event output file");
    declare_parameter("file_leaf_litter", &file_leaf_litter, 300, "leaf litter output file");
    declare_parameter("file_cmass_debt", &file_cmass_debt, 300, "cmass debt output file");
    declare_parameter("file_lo_day", &file_lo_day, 300, "leaf onset day output file");
    declare_parameter("file_resp", &file_resp, 300, "respiration output file");
    //<==
...
void CommonOutput::define_output_tables() {
    ...
    // FROST
    ColumnDescriptors frost_columns;
    frost_columns += ColumnDescriptors(pfts, 8, 3);
    ...
    create_output_table(out_frost,          file_frost,          cmass_columns);
    create_output_table(out_leaf_litter,    file_leaf_litter,    clitter_columns);
    create_output_table(out_cmass_debt,     file_cmass_debt,     cmass_columns);
    create_output_table(out_lo_day,         file_lo_day,         cmass_columns);
    create_output_table(out_resp,           file_resp,           anpp_columns);
    //<==
...
void CommonOutput::outannual(Gridcell& gridcell) {
    ...
    double landcover_frost[NLANDCOVERTYPES]={0.0};
    double landcover_lo_day[NLANDCOVERTYPES]={0.0};
    ...
    double mean_standpft_frost=0.0;
    double mean_standpft_leaf_litter = 0.0;
    double mean_standpft_lo_day = 0.0;
    double mean_standpft_resp = 0.0;
    //<==
    ...
    double frost_gridcell=0.0;
    double leaf_litter_gridcell = 0.0;
    double lo_day_gridcell = 0.0;
    double resp_gridcell = 0.0;
    //<==
    ...
    double standpft_frost=0.0;
    double standpft_leaf_litter=0.0;
    double standpft_lo_day = 0.0;
    double standpft_resp = 0.0;
	...
	while (pftlist.isobj) {
        ...
        mean_standpft_frost=0.0;
        mean_standpft_leaf_litter = 0.0;
        mean_standpft_lo_day = 0.0;
        mean_standpft_resp = 0.0;
		...
		while (gc_itr != gridcell.end()) {
			...
			while (stand.isobj) {
                ...
                // BEN_FROST ==>
                standpft_leaf_litter += patchpft.litter_leaf;
                standpft_resp += patch.fluxes.get_annual_flux(Fluxes::RA, pft.id);
                //<==
					...
					while (vegetation.isobj) {
						...
						if (indiv.id!=-1 && indiv.alive) {
							...
							if (indiv.pft.id==pft.id) {
                                ...
                                patch_frost += indiv.was_frost;
                                standpft_lo_day += indiv.lo_day;
                    ...
                    standpft_frost += patch_frost/vegetation.nobj;
					stand.nextobj();
				} // end of patch loop
                ...
                standpft_frost/=(double)stand.npatch();
                standpft_leaf_litter/=(double)stand.npatch();
                standpft_lo_day/=(double)stand.npatch();
                standpft_resp/=(double)stand.npatch();
                //<==
                ...
                landcover_frost[stand.landcover]+=standpft_frost*stand.get_landcover_fraction();
                landcover_lo_day[stand.landcover]+=standpft_lo_day*stand.get_landcover_fraction();
                ...
                mean_standpft_frost += standpft_frost * stand.get_gridcell_fraction()/active_fraction;
                mean_standpft_leaf_litter += standpft_leaf_litter * stand.get_gridcell_fraction()/active_fraction;
                mean_standpft_lo_day += standpft_lo_day * stand.get_gridcell_fraction()/active_fraction;
                mean_standpft_resp += standpft_resp * stand.get_gridcell_fraction()/active_fraction;
                //<==
                ...
                leaf_litter_gridcell += standpft_leaf_litter*fraction_of_gridcell;
                frost_gridcell += standpft_frost*fraction_of_gridcell;
                lo_day_gridcell += standpft_lo_day*fraction_of_gridcell;
                //<==
        ...
        // BEN_FROST ==>
        outlimit(out,out_cmass_leaf,mean_standpft_cmass_leaf);
        outlimit(out,out_cmass_heart,mean_standpft_cmass_heart);
        outlimit(out,out_cmass_sap,mean_standpft_cmass_sap);
        outlimit(out,out_cmass_root,mean_standpft_cmass_root);
        outlimit(out,out_cmass_debt,mean_standpft_cmass_debt);
        outlimit(out, out_frost, mean_standpft_frost);
        outlimit(out, out_leaf_litter, mean_standpft_leaf_litter);
        outlimit(out, out_lo_day, mean_standpft_lo_day);
        outlimit(out, out_resp, mean_standpft_resp);
        //<==
    ...
    // BEN_FROST ==>
    outlimit(out, out_cmass_leaf,   cmass_leaf_gridcell);
    outlimit(out, out_cmass_heart,  cmass_heart_gridcell);
    outlimit(out, out_cmass_root,   cmass_root_gridcell);
    outlimit(out, out_cmass_sap,    cmass_sap_gridcell);
    outlimit(out, out_frost,       frost_gridcell);
    outlimit(out, out_leaf_litter,  leaf_litter_gridcell);
    outlimit(out, out_cmass_debt,   cmass_debt_gridcell);
    outlimit(out, out_lo_day,       lo_day_gridcell);
    outlimit(out, out_resp,         resp_gridcell);
    //<==
	...
	if (run_landcover) {
		for(int i=0;i<NLANDCOVERTYPES;i++) {
			if(run[i]) {
                ...
                outlimit(out, out_cmass_leaf, landcover_cmass_leaf[i]);
                outlimit(out, out_cmass_heart, landcover_cmass_heart[i]);
                outlimit(out, out_cmass_root, landcover_cmass_root[i]);
                outlimit(out, out_cmass_sap, landcover_cmass_sap[i]);
