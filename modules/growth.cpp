...
double rand_normal(long seed, double mu, double sigma) {

    std::default_random_engine generator(seed);

    std::normal_distribution<double> norm {mu, sigma};
    double val = norm(generator);
...
void leaf_phenology_pft(Pft& pft, Climate& climate, double wscal, double aphen, double& phen, int& sec_can,
        bool& was_frost, double& Rc, double& Rf, double Sc, double Sf, int& lo_stat, int a_lo_stat, long seed) {
    ...
    double frost_tolerance = -1.65; //initialize with mean
    ...
    if (iffrost) {
        ...
        if (sec_can <= 0)
            sec_can = 0;
        if (pft.name == "Fag_syl")
            frost_tolerance = rand_normal(seed, pft.frost_tolerance, pft.frost_sd);
	...
	if (summergreen) {
		...
		if (pft.lifeform == TREE) {
            if (iffrost) {
                if (pft.name == "Fag_syl") { // all params from Kramer et al 2017
                    if (climate.temp < -19.61) { // tc_min
                        Rc = 0;
                    } else if (-19.61 <= climate.temp && climate.temp <= -0.24) { // tc_min, tc_opt
                        Rc = (climate.temp - -19.61) / (-0.24 - -19.61); // tc_min, tc_opt, tc_min
                    } else if (-0.24 <= climate.temp && climate.temp <= 77.13) { // tc_opt, tc_max
                        Rc = (climate.temp - 77.13) / (-0.24 - 77.13); // tc_max, tc_opt, tc_max
                    } else if (climate.temp >= 77.13) {
                        Rc = 0;
                    }

                    if (climate.temp <= 0) { // tf_min
                        Rf = 0;
                    } else if (climate.temp > 0 & Sc > 123) { // tf_min, Sc_thresh
                        Rf = 1 / (1 + exp(-0.1 * (climate.temp + -32.58)));
                    } else if (Sc < 123) { // Sc_thresh
                        Rf = 0;
                    }
			...
			if (climate.gdd5 > pft.gdd0[climate.chilldays] && aphen < APHEN_MAX) {
                if (iffrost) { //BEN_FROST: set phenology to 0 if temperature drops below threshold after leafout
                    if (sec_can == 0) {
                        if (climate.tmin < frost_tolerance && lo_stat >= 1 && a_lo_stat <= 15) {
                            phen = 0.0;
                            sec_can = 60;
                            was_frost = true;
                        }
                        else {
                            phen = min(1.0, (climate.gdd5 - pft.gdd0[climate.chilldays]) / pft.phengdd5ramp);
                        }
                    }
                    else if (sec_can <= 20.0 && sec_can > 0) {
                        phen = min((1.0 / 20.0) * (20.0 - (sec_can - 1)),
...
void leaf_phenology(Patch& patch, Climate& climate) {
	...
	while (patch.pft.isobj) {
		...
		if(patch.stand.pft[ppft.id].active) {
			...
			if(ppft.pft.landcover == CROPLAND && patch.stand.landcover == CROPLAND)
				...
				leaf_phenology_pft(ppft.pft, climate, ppft.wscal, ppft.aphen, ppft.phen, ppft.sec_can, ppft.was_frost,
				        ppft.Rc, ppft.Rf, ppft.Sc, ppft.Sf, ppft.lo_stat, ppft.a_lo_stat, patch.id);
			...
			if ( (climate.lat >= 0.0 && date.day == COLDEST_DAY_NHEMISPHERE) ||
				...
				ppft.aphen = 0.0;
                if (iffrost) {
                    ppft.sec_can = 0; //BEN FROST
                    }
			    }
			//BEN FROST ===>
			if (iffrost) {
                if (date.day == 0) {
                    ppft.was_frost = false;
                    ppft.a_lo_stat = 0;
                }
                if (date.month == 10 && date.dayofmonth == 0) {
                    ppft.Sf = 0.0; //BEN FROST
                    ppft.Sc = 0.0; //BEN FROST
                    ppft.lo_stat = 0;
                }
	...
	while (vegetation.isobj) {
		...
		// For this individual ...
        indiv.sec_can = patch.pft[indiv.pft.id].sec_can;
        indiv.was_frost = patch.pft[indiv.pft.id].was_frost;
        indiv.a_lo_stat = patch.pft[indiv.pft.id].a_lo_stat;
        if (indiv.a_lo_stat == 1) {
            indiv.lo_day = date.day;
        }
        if (indiv.sec_can == 60) {
            indiv.frost_litter = indiv.cmass_leaf_today();
        }
		...
		if (negligible(indiv.densindiv))
			...
			if (!killed) {
				...
				if (indiv.pft.lifeform == TREE) {
					...
					// C debt
					indiv.cmass_debt += (cmass_debt_inc * indiv.densindiv) + indiv.frost_litter;

					// alive check before ensuring C balance
					if (indiv.alive) {
						patch.pft[indiv.pft.id].litter_leaf += (litter_leaf_inc * indiv.densindiv) + indiv.frost_litter;
